...
async function map(mapname, int = require(Interaction)) {
    ...
    if(mapname === "어센트") {
        ...
        int.reply({embeds: [EmbedMap], files: [attachment]})
    } else if (mapname === "바인드") {
        ...
        int.reply({embeds: [EmbedMap], files: [attachment]}).catch(err => { if(err){ console.log(err) } });
    } else if (mapname === "헤이븐") {
